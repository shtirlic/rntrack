...
tm * FLocalTime(time_t * pt)
{
    ...
    for(m.tm_mon = m.tm_mday = 0; m.tm_mday == 0; m.tm_mon++)
    {
        ...
        {
            m.tm_mday = m.tm_yday + 1 -
                        (_mdays[m.tm_mon] + (m.tm_mon != 1 ? LeapDay : 0));
        }
    }
...
int Kludge::operator ==(const Kludge & k) const
{
    ...
    if(_Name != NULL && k._Name != NULL && stricmp(_Name, k._Name) != 0)
    {
        return FALSE;
    }

    if(_Body == NULL && k._Body != NULL)
    {
        return FALSE;
    }
    ...
    if(_Body != NULL && k._Body != NULL && stricmp(_Body, k._Body) != 0)
    {
        return FALSE;
    }
...
void Kludge::Print(void)
{
    if(_Name != NULL)
    {
        Log.Level(LOGD) << "'" << _Name << "'";
    }

    if(_Body != NULL)
    {
        Log.Level(LOGD) << " --- '" << _Body << "'";
    }
...
Kludge::Kludge(char * Txt)
{
    ...
    if(p2 != NULL && strstr(p2, "* Origin:") == p2)  // Work around for ' *
    {
        // Origin:'
        CHP = 314;
...
// Fucked FTN standarts...
    ...
    while(*tmt1 != '\0')
    {
        ...
        if(NewLine == TRUE)  // All kludges should start from beginning of a
        {
            // string.
